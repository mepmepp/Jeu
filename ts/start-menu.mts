import { fadeInMusic, fadeOutMusic, slowlyFadeOutMusic, startAudio, introAudio, outroAudio, boom } from "./audio.mts";
...
let startGameRun: boolean = false;

window.document.body.style.overflow = "hidden";

const introLore: string[] = [
...
window.addEventListener("keydown", (event: KeyboardEvent) => {
  if (event.key === "Enter" || event.key === " ") {
    if (startGameRun === false && startButton) {
...
export async function startGame(): Promise<void> {
  const animationTime = await buttonAnimation();
    fadeInMusic(introAudio);
    const introTime = intro();
    const totalBeforeTime = introTime + animationTime;
    setTimeout(() => {
...
export function endGame(): number {
  ...
  let totalDelay = 0;
  outroLore.forEach((message: string) => {
    let displayTime = letterTime(message);
    displayTime += displayTime < 3000 ? 2000 : displayTime < 4000 ? 500 : 0;
...
function buttonAnimation(): Promise<number> {
...
function setIntroTimeout(fn: () => void, delay: number): number {
  const id = window.setTimeout(fn, delay);
  introTimeouts.push(id);
  return id;
}

function clearIntroTimeouts(): void {
  introTimeouts.forEach((id: number) => clearTimeout(id));
  introTimeouts = [];
}
...
function intro(): number {
  ...
  let totalDelay = 0;
  introLore.forEach((message: string) => {
    let displayTime = letterTime(message);
    displayTime += displayTime < 3000 ? 2000 : displayTime < 4000 ? 500 : 0;
...
function toggleFadeText(element: HTMLElement, time: number): void {
...
function letterTime(text: string): number {
